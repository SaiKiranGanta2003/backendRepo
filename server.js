...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      readableStream.push(req.file.buffer); // Buffer from multer
      ...
      // adding user approver reviwers data for comment table
...
app.post("/routes/documents/resend/:_id", upload, async (req, res) => {
  try {
    ...
    if (updatedDocument.modifiedCount === 0) {
      return res
        .status(404)
        .json({ error: "Document not found or not updated" });
    }

    return res.json({
      message: `Document updated successfully with file: ${fileName}`,
    });
  } catch (err) {
...
app.put("/addReviewersDocument", async (req, res) => {
  try {
    ...
    const Doc = await DocData.updateOne(
      { name: name },
...
app.post("/DocumentsList", async (req, res) => {
  try {
    return res.json(await DocData.find());
  } catch (err) {
    console.log(err.message);
...
app.get("/auditTrail.html", async (req, res) => {
  try {
    const file = fs.readFileSync("./frontend1/auditTrail.html", "utf-8");
    return res.send(file);
    } catch (err) {
    console.log("error in getting file line number 709" + err);
  }
})

app.get("/sing.jpeg", async (req, res) => {
  try {
      // Resolve the file path
      const filePath = path.resolve("./frontend1/sing.jpeg");
      ...
      console.error("Error in getting file (line 524):", err.message);

      // Send an error response
      return res.status(500).send("Error loading the image file.");
  }
});
...
app.get("/demo.html",async(req,res)=>{
  try{
    const file = fs.readFileSync("./frontend1/demo.html", "utf-8");
    return res.send(file);
  }
  catch(err){
    console.log("error in getting file line number 524" + err);
  ...
  try {
    ...
    if (!documentName || !Array.isArray(comments) || comments.length === 0) {
      return res
        .status(400)
        .send(
          "Invalid data. Please provide a document name and an array of comments."
        );
    }
    ...
    if (existingDocument) {
      ...
      return res
        .status(200)
        .send("Comments added successfully to the existing document.");
    } else {
      ...
      return res
        .status(200)
        .send("New document created and comments added successfully.");
    }
  } catch (error) {
    console.error(error);
...
app.get("/status.html", async (req, res) => {
  try {
    const file = fs.readFileSync("./frontend1/status.html", "utf-8");
    return res.send(file);
  } catch (err) {
